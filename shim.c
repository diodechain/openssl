...
long X_SSL_get0_ec_point_formats(SSL *ssl, char** plist) {
    if (!ssl->ext.ecpointformats) {
        return 0;
    }
    *plist = ssl->ext.ecpointformats;
    return ssl->ext.ecpointformats_len;
}

long X_SSL_set0_ec_point_formats(SSL *ssl, char* list, size_t length) {
    if (ssl->ext.ecpointformats) {
        OPENSSL_free(ssl->ext.ecpointformats);
    }

    ssl->ext.ecpointformats = BUF_memdup(list, length);
    if (!ssl->ext.ecpointformats) {
        ssl->ext.ecpointformats_len = 0;
        return 0;
    }
    ssl->ext.ecpointformats_len = length;
    return length;
}
...
int static logit(const char *str, size_t len, void*u) {
    fputs(str, stderr);
    return 0;
}

void X_ERR_print_errors() {
    ERR_print_errors_cb(logit, 0);
}
