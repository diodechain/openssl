...
func newTCPConn(conn *net.TCPConn, ctx *Ctx) (*TCPConn, error) {
	...
	c := &TCPConn{
		...
		into_ssl: into_ssl,
		from_ssl: from_ssl}
	runtime.SetFinalizer(c, func(c *TCPConn) {
...
// TCPClient wraps an existing stream connection and puts it in the connect state
...
func TCPClient(conn *net.TCPConn, ctx *Ctx) (*TCPConn, error) {
	c, err := newTCPConn(conn, ctx)
	if err != nil {
		return nil, err
	}
...
// TCPServer wraps an existing stream connection and puts it in the accept state
// for any subsequent handshakes.
func TCPServer(conn *net.TCPConn, ctx *Ctx) (*TCPConn, error) {
	c, err := newTCPConn(conn, ctx)
	if err != nil {
		return nil, err
	}
...
func (c *TCPConn) GetCtx() *Ctx { return c.ctx }

func (c *TCPConn) CurrentCipher() (string, error) {
...
func (c *TCPConn) fillInputBuffer() error {
	for {
...
func (c *TCPConn) flushOutputBuffer() error {
	_, err := c.from_ssl.WriteTo(c.conn)
	return err
}

func (c *TCPConn) getErrorHandler(rv C.int, errno error) func() error {
...
func (c *TCPConn) handleError(errcb func() error) error {
	if errcb != nil {
		return errcb()
	}
	return nil
...
func (c *TCPConn) handshake() func() error {
	c.mtx.Lock()
	defer c.mtx.Unlock()
	if c.is_shutdown {
...
func (c *TCPConn) PeerCertificate() (*Certificate, error) {
	c.mtx.Lock()
	defer c.mtx.Unlock()
	if c.is_shutdown {
...
func (c *TCPConn) loadCertificateStack(sk *C.struct_stack_st_X509) (
	rv []*Certificate) {
...
// PeerCertificate.
func (c *TCPConn) PeerCertificateChain() (rv []*Certificate, err error) {
	c.mtx.Lock()
	defer c.mtx.Unlock()
	if c.is_shutdown {
		return nil, errors.New("connection closed")
...
func (c *TCPConn) ConnectionState() (rv ConnectionState) {
...
func (c *TCPConn) shutdown() func() error {
	c.mtx.Lock()
	defer c.mtx.Unlock()
	runtime.LockOSThread()
	defer runtime.UnlockOSThread()
...
func (c *TCPConn) shutdownLoop() error {
	err := tryAgain
	shutdown_tries := 0
	for err == tryAgain {
...
func (c *TCPConn) read(b []byte) (int, func() error) {
	if len(b) == 0 {
		return 0, nil
	}
	c.mtx.Lock()
...
func (c *TCPConn) write(b []byte) (int, func() error) {
	if len(b) == 0 {
		return 0, nil
	}
	c.mtx.Lock()
...
// SSLRecordSize.
func (c *TCPConn) Write(b []byte) (written int, err error) {
	if len(b) == 0 {
		return 0, nil
	}
	err = tryAgain
...
// certificate.
func (c *TCPConn) VerifyHostname(host string) error {
...
// SetKeepAlive calls SetKeepAlive on the underlying connection.
func (c *TCPConn) SetKeepAlive(keepalive bool) error {
	return c.conn.SetKeepAlive(keepalive)
}

// SetKeepAlivePeriod calls SetKeepAlivePeriod on the underlying connection.
...
func (c *TCPConn) UnderlyingConn() *net.TCPConn {
	return c.conn
}

func (c *TCPConn) SetTlsExtHostName(name string) error {
...
func (c *TCPConn) VerifyResult() VerifyResult {
	return VerifyResult(C.SSL_get_verify_result(c.ssl))
}

func (c *TCPConn) SessionReused() bool {
	return C.X_SSL_session_reused(c.ssl) == 1
}

func (c *TCPConn) GetSession() ([]byte, error) {
	runtime.LockOSThread()
	defer runtime.UnlockOSThread()
...
func (c *TCPConn) setSession(session []byte) error {
	runtime.LockOSThread()
	defer runtime.UnlockOSThread()
